...
async def _fetch_lark_events(
    *,
    ...
    start_ts = str(int(window_start_utc.astimezone(timezone.utc).timestamp()))
    end_ts = str(int(window_end_utc.astimezone(timezone.utc).timestamp()))
...
async def run() -> None:
    ...
    async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as google_client, \
        ...
        if cfg.debug_enabled:
            ...
            try:
                debug_events = await _fetch_lark_events(
